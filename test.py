...
if TELEGRAM_SUPPORT_CHAT_ID is None or not str(TELEGRAM_SUPPORT_CHAT_ID).lstrip("-").isdigit():
    raise Exception(
        "You need to specify 'TELEGRAM_SUPPORT_CHAT_ID' env variable:"
        "The bot will forward all messages to this chat_id."
    )
...
async def echo(bot: Bot, update_id: int) -> int:
    ...
    for update in updates:
        ...
        if update.message and update.message.text:
            ...
            topic = await bot.createForumTopic(TELEGRAM_SUPPORT_CHAT_ID, name=update.effective_user.username)
            logger.info("Found message %s!", update.message.text + topic.name + str(topic.message_thread_id))
